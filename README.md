...
一个中文、自助式 AI 学习工具：目标 → 路线 → 课程 → 练习与 AI 答疑 → 个人 Wiki。提供 Windows 桌面版，也保留 Web 开发模式。

## Windows 桌面版

当前源码版本为 `1.0.1`，对应 Git 标签 `V-1.0.1`。打包后运行 `release/Learnflow-Setup-1.0.1.exe` 安装。也可直接运行 `release/win-unpacked/Learnflow.exe`（需保留同目录的其他文件，不能只拷贝 exe）。安装后从桌面“知行 Learnflow”快捷方式打开，无需安装 Node.js，也不用手动启动 Web 服务。旧版 `0.1.0` 安装包不包含新增的 SQLite 存储；请重新打包或用开发测试版体验最新功能。
...
开发测试版会打开模型设置页，使用项目中的 `.desktop-dev/data/` 保存数据，与已安装版本和自动化测试数据分开，可以同时运行。首次运行新版时，如果存在旧 `learning.json`，会校验并导入 SQLite；原 JSON 和 `.bak` 原样保留。它不会自动导入正式版模型配置或学习记录，需要在测试版填写配置，或手动导入学习备份。源码新增功能会先出现在开发测试版；已有安装包不会自动更新。

```powershell
npm run desktop:build
```

`desktop:build` 生成 Windows x64 安装包；`desktop:pack` 只生成解包目录。开发/构建需要 Node.js 22.16 或更高版本。首次安装依赖和打包需要联网下载 Electron、NSIS 等工具；应用运行时不需要这些构建工具。当前安装包未做代码签名。
...
新生成的 AI 课程采用分步模式：先生成本课大纲，再按需点击每个内容块单独生成讲解、示例、实践、练习或总结；还可以在课程末尾添加自定义内容块。已生成的内容块立即单独保存，未生成的块只保存标题和目标。旧版固定结构课程继续原样阅读、练习，无需重生成。

学习时进入某节课程的“AI 答疑”标签，可围绕课程讲解、示例或练习连续提问。AI 会参考本课内容和最近 12 条对话；每节课保留最近 20 条问答消息，桌面版保存在本机学习记录中，Web 版保存在当前浏览器。没有可用模型时，课程仍可阅读和练习，但无法进行 AI 答疑。云端模型答疑会将本课内容、问题和最近对话发送给已配置的服务商。
...
- `backups/`：导入 JSON 备份或从旧版 SQLite 升级结构前自动创建的 SQLite 快照。`settings.json.bak` 仍只保留上次设置保存前的版本。
...
需要 Node.js 22.16 或更高版本。Web 服务本身没有第三方运行依赖；桌面开发需要执行 `npm install` 安装开发依赖。
